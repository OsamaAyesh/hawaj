import 'package:app_mobile/core/extensions/extensions.dart';
import 'package:app_mobile/core/resources/manager_mockup.dart';
import 'package:app_mobile/core/response/with_out_data_response.dart';
import 'package:app_mobile/features/common/auth/data/request/send_otp_request.dart';
import 'package:app_mobile/features/common/auth/data/response/send_otp_response.dart';
import 'package:app_mobile/features/providers/offers_provider/manage_list_offer/data/response/offer_response.dart';
import 'package:flutter_dotenv/flutter_dotenv.dart';
import '../../../../../../constants/env/env_constants.dart';
import '../../../../../../core/network/app_api.dart';
import 'dart:convert';
import 'package:flutter/services.dart' as rootBundle;

import '../../../../../../core/util/get_app_langauge.dart';
import '../response/offer_user_response.dart';



abstract class GetOffersUserDataSource{
  Future<OfferUserResponse> getOffersData();
}

class GetOffersUserDataSourceImplement
    implements GetOffersUserDataSource {
  AppService _appService;

  GetOffersUserDataSourceImplement(this._appService);

  @override
  Future<OfferUserResponse> getOffersData(
      ) async {
    if (dotenv.env[EnvConstants.debug].onNullBool()) {
      return OfferUserResponse.fromJson(
        json.decode(
          await rootBundle.rootBundle.loadString(
            ManagerMokUp.login,
          ),
        ),
      );
    }
    return await _appService.getOffers(
        AppLanguage().getCurrentLocale(),
    );
  }
}
